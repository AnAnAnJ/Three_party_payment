...
class TestSearchValue:
    ...
    def test_search_value(self):
        ...
        """
        search_value = sql_def(sql_query = f'SELECT `bundle_id`, `app_project_name`,`app_name` FROM `joymeet`.`app_package` where `bundle_id` = {self.bundle_id};')
    ...
    def test_admin_goods(self):
        ...
        """
        try:
            if self.test_search_value[1] == "viva":
            ...
            elif self.test_search_value[1]  == "joy meet":
                sql  = Test_Query_admin_data()
                res = sql.read_mysql(f"待维护{self.page_name}")
                return res
            else:
